...
class Cylinder:
    def __init__(self, n_tick, min_tick, low_limit, high_limit, ob_depth = 5):
        ...
        # This record the latest ob
        self.last_row = [None] * self.n_level

        # this list record the distance from the last valid number corresponding to "record"
        self.distance = [deque([None] * n_tick) for i in range(self.n_level)]
        # This record the latest distance
        self.last_dist = [None] * self.n_level
    ...
    def update(self, tick):
        for i in range(len(self.last_dist)):
            if self.last_dist[i] is not None:
                self.last_dist[i] = self.last_dist[i] + 1
        for i in range(5, 0, -1):
            ...
            if i == 5:
                ...
                self.last_dist[bid_posit: ask_posit + 1] = [0] * (ask_posit - bid_posit + 1)
        ...
        for i in range(self.n_level):
            self.distance[i].popleft()
            self.distance[i].append(self.last_dist[i])
    ...
    def retrieve_dist(self, price):
        posit = int(round((price - self.low_limit) * self.min_tick_div))
        return self.distance[posit]
...
if __name__ == '__main__':
    ...
    for index, row in priceData.iterrows():
        ...
        print(cy.retrieve_dist(row["BidPrice1"]))
        input()
