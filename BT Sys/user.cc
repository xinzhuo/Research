...
namespace lhr_strategy
{
	...
	void TD::update_new_tick(Tick *tick) { //on tick
		...
		this->sys_time = tick->system_time_ns;
	...
	void CD::update_movenext() {
		...
		int previous_n_position = this->position - this->std_window - 1;
		if (previous_n_position < 0) {
	...
	void CD::update() { // on tick
		...
		int previous_10_position = this->position - 10;
		if (previous_10_position < 0) {
			previous_10_position += this->size;
		}
		...
		this->sprd = round((this->td->ask_price_1[this->position] - this->td->bid_price_1[this->position]) * cs->cp->inverse_min_step);
		this->ret_in_tick = (this->td->ask_price_1[this->position] + this->td->bid_price_1[this->position]
			- this->td->ask_price_1[previous_10_position] - this->td->bid_price_1[previous_10_position]) / 2 * cs->cp->inverse_min_step;
		//tavp
	...
	void CD::show() {
		...
		sprintf(b, "[Trigger_info], index: %d, name: %s, tavp: %f, side: %s, bid: %f, ask: %f, bid_stdev: %f, ask_stdev: %f, bid_widen: %f, ask_widen: %f, volume: %d, sprd: %f, ret_in_tick: %f, timestamp: %ld",
			cs->cd->counter, cs->name, cs->cd->tavp, cs->cd->tag, cs->td->bid_price_1[cs->td->position], cs->td->ask_price_1[cs->td->position],
			cs->cd->bid_stdev, cs->cd->ask_stdev,
			round((cs->td->last_bid_price_1 - cs->td->bid_price_1[cs->td->position]) * cs->cp->inverse_min_step),
			round((cs->td->ask_price_1[cs->td->position] - cs->td->last_ask_price_1) * cs->cp->inverse_min_step),
			cs->cd->tick_volume[cs->cd->position],
			cs->cd->sprd,
			cs->cd->ret_in_tick,
			cs->td->sys_time
		);
	...
	void strategy_prepare(CS *cs) { //strategy, generate expect position and expect price or priority
		cs->cd->tag = "NAN";
		if (cs->cd->counter < 120) {
			return;
		}
		// Bid side widen
		if (round((cs->td->last_bid_price_1 - cs->td->bid_price_1[cs->td->position]) * cs->cp->inverse_min_step) >= 4
			&& AreSame(cs->td->last_ask_price_1, cs->td->ask_price_1[cs->td->position])) {
			if (cs->cd->tick_volume[cs->cd->position] <= 10 + EPSILON &&
				cs->cd->ret_in_tick > -3.75 &&
				cs->cd->sprd > 6 + EPSILON && 
				cs->cd->ask_stdev <= 2.363 * cs->cp->step) {
				cs->cd->tag = "buy";
				//char s[512];
				//sprintf(s, "[Trigger_info] index: %d name: %s tavp: %f side: %s bid: %f ask: %f bid_stdev: %f ask_stdev: %f" ,
				//	cs->cd->counter, cs->name, cs->cd->tavp, "buy", cs->td->bid_price_1[cs->td->position], cs->td->ask_price_1[cs->td->position], 
				//	cs->cd->bid_stdev, cs->cd->ask_stdev);
				//WL(s);
			}
		}
			...
			&& AreSame(cs->td->last_bid_price_1, cs->td->bid_price_1[cs->td->position])) {
			if (cs->cd->tick_volume[cs->cd->position] <= 10 + EPSILON &&
				cs->cd->ret_in_tick < 3.75 &&
				cs->cd->sprd > 6 + EPSILON &&
				cs->cd->bid_stdev <= 2.363 * cs->cp->step) {
				//char s[512];
				//sprintf(s, "[Trigger_info] index: %d name: %s tavp: %f side: %s bid: %f ask: %f bid_stdev: %f ask_stdev: %f",
				//	cs->cd->counter, cs->name, cs->cd->tavp, "buy", cs->td->bid_price_1[cs->td->position], cs->td->ask_price_1[cs->td->position],
				//	cs->cd->bid_stdev, cs->cd->ask_stdev);
...
int on_init(StrategyConfig *config) {
	for (uint32_t i = 0; i < config->contracts_num; ++i) {
		...
		this_cp->inverse_min_step = 1.0 / config->contracts[i].mini_move;
		this_cp->inverse_multiplier = 1.0 / config->contracts[i].multipe;
