...
class Strategy:
    ...
    def process_tick(self, loc):
        ...
        current_time = datetime.fromtimestamp(int(str(ts)[:10]))
        current_time = current_time + timedelta(
            milliseconds=int(str(ts)[10:13]))
        ...
        if self.bData.iloc[self.index + 1]["hCount"] != self.bData.iloc[self.index]["hCount"] and self.cancel_all_eod:
...
class Backtester:
    def __init__(self, hrb, name, mode, filled = 0):
        ...
        self.trimmed_names_l1 = ["AskPrice1", "AskSize1", "BidPrice1", "BidSize1",
                                 "TotalVolume", "Turnover", "TimeStamp", "LastPrice",
                                 "hCount", "dCount", "volume", "turnover", "vwap"]
    ...
    def match_old_orders_l1(self, tick1, tick2):
        current_time = datetime.fromtimestamp(int(str(tick2["TimeStamp"])[:10]))
        current_time = current_time + timedelta(
            milliseconds=int(str(tick2["TimeStamp"])[10:13]))
        time_tuple = self.tick_count, current_time
    ...
    def reduce_between_ticks(self, tick1, tick2, tick1_dict, tick2_dict):
        ...
        if len(price_list) in [3, 4]:
            ...
            if solution3:
                if self.tick_count == 67: print(solution3)
    ...
    def build_tick_trade_record(self, tick2):
        trade_record_last = self.strategy.trade_record_history[self.tick_count - 1].get_key_info()
        ...
        if len(self.strategy.trade_record_on_tick) == 0 or tick2["BidPrice1"] == 0 or tick2["AskPrice1"] == 0:
            self.strategy.trade_record_history[self.tick_count] = TickTradeRecord(tick2["TimeStamp"], *trade_record_last)
            if not(tick2["BidPrice1"] == 0 or tick2["AskPrice1"] == 0):
                self.strategy.trade_record_history[self.tick_count].pnl = pnl_on_tick
            return
        ...
        self.strategy.trade_record_history[self.tick_count] = TickTradeRecord(tick2["TimeStamp"],
                                                         position_on_tick, cashflow_on_tick, pnl_on_tick,
                                                         fee_on_tick)
        self.strategy.trade_record_history[self.tick_count].order_dir = sum_dir_on_tick
        self.strategy.trade_record_history[self.tick_count].volume = volume_on_tick

        if self.strategy.cancelled_count > self.cancelled_count:
            #print("Order cancelled")
            self.strategy.trade_record_history[self.tick_count].cancel_flag = 1
    ...
    def compute_pnl_summary(self, include_fee=True, show_orders=True, show_fee=True, show_cancel = True):
        ...
        df["ts_str"] = df.apply(timestamp_to_str, axis = 1)
    ...
    def run(self, strategy):
        ...
        self.tick_count = 0 # Keep track of index position
        ...
        for row in self.bDataV:
            ...
            if self.data_type == "l2":
                if (self.hrb.tInfo.fSymbol).lower() in ["ic", "if", "ih"]:
                    self.match_old_orders_index(last_tick, this_tick, self.tick_count)
                else:
            ...
            # Feed new tick to strategy:
            new_order_list, temp_order_list = self.strategy.process_tick(self.tick_count)
